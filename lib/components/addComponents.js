...
module.exports = function ({ app, components }) {
  ...
  const fsPlane = components.add(getFullScreenPlane(app, {
    uniforms: {
      noiseMap: {
        type: 't', value: new THREE.TextureLoader().load('img/noise.png', (tex) => {
        tex.minFilter = THREE.LinearFilter;
        tex.generateMipmaps = false;
        tex.needsUpdate = true;
      }) },
      heightMap: {
        type: 't', value: new THREE.TextureLoader().load('img/heightmap02.png', (tex) => {
          tex.minFilter = THREE.LinearFilter;
          tex.generateMipmaps = false;
          tex.needsUpdate = true;
        })
      ...
      colorPalette: {
        type: 't', value: new THREE.TextureLoader().load('img/color-palette.png', (tex) => {
          tex.minFilter = THREE.LinearFilter;
          tex.generateMipmaps = false;
          tex.needsUpdate = true;
        })
      ...
      paletteX: { type: 'f', value: Math.random() }
    }
   }));
